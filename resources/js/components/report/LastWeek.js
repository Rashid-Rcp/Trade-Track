...
function LastWeek() {
  ...
  const lastWeek = moment().startOf('isoWeek').subtract(7, 'days');
 
  let rowData =[];
  trades.forEach((trade)=>{
    let same = moment(trade.date).isSame(lastWeek);
    let after =  moment(trade.date).isAfter(lastWeek);
    let before =  moment(trade.date).isBefore(moment().startOf('isoWeek'));
